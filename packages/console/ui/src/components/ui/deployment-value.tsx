  ...
  return (
    <div className={cn("h-12 px-4 border rounded-md relative overflow-x-hidden", props.className)}>
      <div className="flex items-start justify-center flex-col h-full">
        ...
        <span className="text-xs text-muted-foreground">{capitalize(props.description)}</span>
      </div>
      <button
        className="absolute right-0 top-1/2 -translate-y-1/2 rounded-sm h-full w-12 border grid place-content-center bg-gray-100"
        onClick={onCopyClick}
      >
        {isCopied ? (
