...
import { FulfillModal } from "./modals/fulfill";

export const NewOrdersTable: React.FC<{ orders: Order[] }> = ({ orders }) => {
  return (
          ...
          <tbody>
            ...
            {orders.map((o) => (
              <tr key={o.id} className="border-b border-gray-100">
                ...
                <td className="h-16 px-4 text-gray-900">
                  {new Date(Number(o.createdAt) * 1000).toISOString().slice(0, 16).replace("T", " ")}
                </td>
                <td className="h-16 px-4 text-gray-900">{o.client}</td>
                <td className="h-16 px-4 text-right">
                  <FulfillModal order={o}>
                    <Button
                      variant="outline"
